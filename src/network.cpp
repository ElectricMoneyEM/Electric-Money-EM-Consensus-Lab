...
void P2PNode::sync_once() {
    ...
    /*
     * If the common ancestor is our current tip,
     * there is no known newer block on the peer's chain.
     *
     * We still avoid replacing our chain with an empty
     * suffix.
     */
    if(common_index==
       static_cast<int>(local.size()-1)) {
